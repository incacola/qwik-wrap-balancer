...
export const Balancer = component$((props: BalancerProps) => {
  ...
  const relayout = $((id: string, ratio: number, wrapper: WrapperElement) => {
    ...
    if (!wrapper["__wrap_o"]) {
      (wrapper["__wrap_o"] = new ResizeObserver(() => {
        (self as any).__wrap_b(0, +(wrapper.dataset?.brr ?? 0), wrapper);
      })).observe(container);
    }}
  ...
  useVisibleTask$(({ track }) => {
    ...
    if (wrapperRef.value) {
      ...
      ((self as any)[SYMBOL_KEY] = relayout)("balancer", ratio, wrapperRef.value);
    }
  });
